...
int main(int argc, char*argv[])
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        //glm::mat4 rotationMatrix = glm::rotate(translationMatrix, glm::radians(angle)+radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        // car vector 
        float radius = 2.0f;
        ...
        // the rotation matrix does not function for some reason
        glm::mat4 rotationMatrix = glm::rotate(translationMatrix, glm::radians(angle), glm::vec3(0.0f, 1.0f, 0.0f));

        // Draw simple car to go in circles
        glBindTexture(GL_TEXTURE_2D, cementTextureID);
        mat4 carWorldMatrix = translate(mat4(1.0f), vec3(1.0f, 0.0f, 1.0f)) * scale(mat4(1.0f), vec3(2.0f, 1.0f, 1.0f));
        carWorldMatrix = carWorldMatrix * translationMatrix * rotationMatrix;
        setWorldMatrix(texturedShaderProgram, carWorldMatrix);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        
        // drawing a top part relative to the bottom part of the car. 
        glBindTexture(GL_TEXTURE_2D, brickTextureID);
        mat4 carTOPWorldMatrix = carWorldMatrix * scale(mat4(1.0f), vec3(0.5f, 0.25f, 1.0f)) * translate(mat4(1.0f), vec3(0.0f,2.5f,0.0f));
...
GLuint loadTexture(const char *filename) {
    ...
    if (!data) {
        std:cerr << "Error::Texture could not load texture files" << filename << std::endl;
        return 0;
    }
